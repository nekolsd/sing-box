...
func TestSniffQUICQuicGoFingerprint(t *testing.T) {
	...
	case packets := <-packetsChan:
		...
		for i, pkt := range packets {
			...
			t.Logf("Packet %d: err=%v, sniff_host=%s, client=%s", i, err, metadata.SniffHost, metadata.Client)
		...
		t.Logf("SniffHost: %s", metadata.SniffHost)
		...
		if len(packets) > 0 {
			...
			if errors.Is(err, sniff.ErrNeedMoreData) && len(packets) > 1 {
				...
				for i := 1; i < len(packets); i++ {
					...
					t.Logf("Packet %d sniff result: err=%v, sniff_host=%s, sniffCtx=%v", i, err, metadata.SniffHost, metadata.SniffContext != nil)
					if metadata.SniffHost != "" || (err != nil && !errors.Is(err, sniff.ErrNeedMoreData)) {
						break
					}
				}
			}
			...
			// Log final results
			t.Logf("Final: Protocol=%s, SniffHost=%s, Client=%s", metadata.Protocol, metadata.SniffHost, metadata.Client)

			// Verify SNI extraction
			if metadata.SniffHost == "" {
				t.Errorf("Failed to extract SNI, expected: %s", testSNI)
			} else {
				require.Equal(t, testSNI, metadata.SniffHost, "SNI should match")
			}
